...
set.seed(42)
network_plot <- ggraph(graph, layout = "fr", niter = 5000, start.temp = 0.01) + 
  geom_edge_link(
  ...
  labs(title = "Keyword Network Analysis: Global Risks") +
  expand_limits(x = c(-2, 2), y = c(-2, 2))  # More space for spread
...
ggsave(
  "keyword_network_global_risks.svg",
  plot = network_plot,
  device = svglite,
