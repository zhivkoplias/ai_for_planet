...
#plot3 rayyan's predictions
# Load ggplot2 package  
library(ggplot2)  

# Create the data  
data <- data.frame(  
  Category = c("Most likely to include", "Likely to include", "No recommendation",   
               "Likely to exclude", "Most likely to exclude"),  
  Count = c(1621, 5058, 9560, 1973, 1766)  
)  

# Reorder the categories to place "No recommendation" in the middle  
data$Category <- factor(data$Category,   
                        levels = c("Most likely to include", "Likely to include",   
                                   "No recommendation",   
                                   "Likely to exclude", "Most likely to exclude"))  

# Define custom colors for the categories  
custom_colors <- c(  
  "Most likely to include" = "#A8DADC",  
  "Likely to include" = "#A8DADC",  
  "No recommendation" = "#457B9D",  
  "Likely to exclude" = "#E63946",  
  "Most likely to exclude" = "#E63946"  
)  
...
ggplot(data, aes(x = Category, y = Count, fill = Category)) +  
  geom_bar(stat = "identity", color = "black", width = 0.7) +  # Bar plot  
  scale_fill_manual(values = custom_colors) +  # Apply custom colors  
  theme_minimal(base_size = 18) +  # Clean theme  
...
#ggsave("distribution_of_categories.svg", plot = plot, width = 16, height = 12, dpi = 300)  